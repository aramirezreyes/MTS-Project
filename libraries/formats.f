...
c-----------------------------------------------------------------------
c$$$      subroutine csrdia (n,idiag,job,a,ja,ia,ndiag,
c$$$     *                   diag,ioff,ao,jao,iao,ind)
c$$$      real*8 diag(ndiag,idiag), a(*), ao(*)
c$$$      integer ia(*), ind(*), ja(*), jao(*), iao(*), ioff(*)
c$$$c----------------------------------------------------------------------- 
c$$$c Compressed sparse row     to    diagonal format
c$$$c----------------------------------------------------------------------- 
c$$$c this subroutine extracts  idiag diagonals  from the  input matrix a,
c$$$c a, ia, and puts the rest of  the matrix  in the  output matrix ao,
c$$$c jao, iao.  The diagonals to be extracted depend  on the  value of job
c$$$c (see below for details.)  In  the first  case, the  diagonals to be
c$$$c extracted are simply identified by  their offsets  provided in ioff
c$$$c by the caller.  In the second case, the  code internally determines
c$$$c the idiag most significant diagonals, i.e., those  diagonals of the
c$$$c matrix which  have  the  largest  number  of  nonzero elements, and
c$$$c extracts them.
c$$$c----------------------------------------------------------------------- 
c$$$c on entry:
c$$$c---------- 
c$$$c n	= dimension of the matrix a.
c$$$c idiag = integer equal to the number of diagonals to be extracted. 
c$$$c         Note: on return idiag may be modified.
c$$$c a, ja, 			
c$$$c    ia = matrix stored in a, ja, ia, format
c$$$c job	= integer. serves as a job indicator.  Job is better thought 
c$$$c         of as a two-digit number job=xy. If the first (x) digit
c$$$c         is one on entry then the diagonals to be extracted are 
c$$$c         internally determined. In this case csrdia exctracts the
c$$$c         idiag most important diagonals, i.e. those having the largest
c$$$c         number on nonzero elements. If the first digit is zero
c$$$c         then csrdia assumes that ioff(*) contains the offsets 
c$$$c         of the diagonals to be extracted. there is no verification 
c$$$c         that ioff(*) contains valid entries.
c$$$c         The second (y) digit of job determines whether or not
c$$$c         the remainder of the matrix is to be written on ao,jao,iao.
c$$$c         If it is zero  then ao, jao, iao is not filled, i.e., 
c$$$c         the diagonals are found  and put in array diag and the rest is
c$$$c         is discarded. if it is one, ao, jao, iao contains matrix
c$$$c         of the remaining elements.
c$$$c         Thus:
c$$$c         job= 0 means do not select diagonals internally (pick those
c$$$c                defined by ioff) and do not fill ao,jao,iao
c$$$c         job= 1 means do not select diagonals internally 
c$$$c                      and fill ao,jao,iao
c$$$c         job=10 means  select diagonals internally 
c$$$c                      and do not fill ao,jao,iao
c$$$c         job=11 means select diagonals internally 
c$$$c                      and fill ao,jao,iao
c$$$c 
c$$$c ndiag = integer equal to the first dimension of array diag.
c$$$c
c$$$c on return:
c$$$c----------- 
c$$$c
c$$$c idiag = number of diagonals found. This may be smaller than its value 
c$$$c         on entry. 
c$$$c diag  = real array of size (ndiag x idiag) containing the diagonals
c$$$c         of A on return
c$$$c          
c$$$c ioff  = integer array of length idiag, containing the offsets of the
c$$$c   	  diagonals to be extracted.
c$$$c ao, jao
c$$$c  iao  = remainder of the matrix in a, ja, ia format.
c$$$c work arrays:
c$$$c------------ 
c$$$c ind   = integer array of length 2*n-1 used as integer work space.
c$$$c         needed only when job.ge.10 i.e., in case the diagonals are to
c$$$c         be selected internally.
c$$$c
c$$$c Notes:
c$$$c-------
c$$$c    1) The algorithm is in place: ao, jao, iao can be overwritten on 
c$$$c       a, ja, ia if desired 
c$$$c    2) When the code is required to select the diagonals (job .ge. 10) 
c$$$c       the selection of the diagonals is done from left to right 
c$$$c       as a result if several diagonals have the same weight (number 
c$$$c       of nonzero elemnts) the leftmost one is selected first.
c$$$c-----------------------------------------------------------------------
c$$$      job1 = job/10
c$$$      job2 = job-job1*10
c$$$      if (job1 .eq. 0) goto 50
c$$$      n2 = n+n-1
c$$$      call infdia(n,ja,ia,ind,idum)
c$$$c----------- determine diagonals to  accept.---------------------------- 
c$$$c----------------------------------------------------------------------- 
c$$$      ii = 0
c$$$ 4    ii=ii+1
c$$$      jmax = 0
...
c$$$      ioff(ii) = i-n
c$$$      ind(i) = - jmax
c$$$      if (ii .lt.  idiag) goto 4
c$$$ 42   idiag = ii
c$$$c---------------- initialize diago to zero ----------------------------- 
c$$$ 50   continue
c$$$      do 55 j=1,idiag
c$$$         do 54 i=1,n
c$$$            diag(i,j) = 0.0d0
...
c$$$      ko = 1
c$$$c----------------------------------------------------------------------- 
c$$$c extract diagonals and accumulate remaining matrix.
c$$$c----------------------------------------------------------------------- 
c$$$      do 6 i=1, n
c$$$         do 51 k=ia(i),ia(i+1)-1 
c$$$            j = ja(k)
c$$$            do 52 l=1,idiag
c$$$               if (j-i .ne. ioff(l)) goto 52
c$$$               diag(i,l) = a(k)
c$$$               goto 51
c$$$ 52         continue
c$$$c--------------- append element not in any diagonal to ao,jao,iao ----- 
c$$$            if (job2 .eq. 0) goto 51
c$$$            ao(ko) = a(k)
c$$$            jao(ko) = j
c$$$            ko = ko+1
c$$$ 51      continue
c$$$         if (job2 .ne. 0 ) ind(i+1) = ko
c$$$ 6    continue
c$$$      if (job2 .eq. 0) return
c$$$c     finish with iao
c$$$      iao(1) = 1
c$$$      do 7 i=2,n+1
c$$$         iao(i) = ind(i)
...
c-----------------------------------------------------------------------
c$$$      subroutine csrbnd (n,a,ja,ia,job,abd,nabd,lowd,ml,mu,ierr)
c$$$      real*8 a(*),abd(nabd,n)
c$$$      integer ia(n+1),ja(*)
c$$$c----------------------------------------------------------------------- 
c$$$c   Compressed Sparse Row  to  Banded (Linpack ) format.
c$$$c----------------------------------------------------------------------- 
c$$$c this subroutine converts a general sparse matrix stored in
c$$$c compressed sparse row format into the banded format. for the
c$$$c banded format,the Linpack conventions are assumed (see below).
c$$$c----------------------------------------------------------------------- 
c$$$c on entry:
c$$$c----------
c$$$c n	= integer,the actual row dimension of the matrix.
c$$$c
c$$$c a,
c$$$c ja,
c$$$c ia    = input matrix stored in compressed sparse row format.
c$$$c
c$$$c job	= integer. if job=1 then the values of the lower bandwith ml 
c$$$c         and the upper bandwidth mu are determined internally. 
c$$$c         otherwise it is assumed that the values of ml and mu 
c$$$c         are the correct bandwidths on input. See ml and mu below.
c$$$c
c$$$c nabd  = integer. first dimension of array abd.
c$$$c
c$$$c lowd  = integer. this should be set to the row number in abd where
c$$$c         the lowest diagonal (leftmost) of A is located. 
c$$$c         lowd should be  ( 1  .le.  lowd  .le. nabd).
c$$$c         if it is not known in advance what lowd should be
c$$$c         enter lowd = 0 and the default value lowd = ml+mu+1
c$$$c         will be chosen. Alternative: call routine getbwd from unary
c$$$c         first to detrermione ml and mu then define lowd accordingly.
c$$$c         (Note: the banded solvers in linpack use lowd=2*ml+mu+1. )
c$$$c
c$$$c ml	= integer. equal to the bandwidth of the strict lower part of A
c$$$c mu	= integer. equal to the bandwidth of the strict upper part of A
c$$$c         thus the total bandwidth of A is ml+mu+1.
c$$$c         if ml+mu+1 is found to be larger than lowd then an error 
c$$$c         flag is raised (unless lowd = 0). see ierr.
c$$$c
c$$$c note:   ml and mu are assumed to have	 the correct bandwidth values
c$$$c         as defined above if job is set to zero on entry.
c$$$c
c$$$c on return:
c$$$c-----------
c$$$c
c$$$c abd   = real array of dimension abd(nabd,n).
c$$$c         on return contains the values of the matrix stored in
c$$$c         banded form. The j-th column of abd contains the elements
c$$$c         of the j-th column of  the original matrix comprised in the
c$$$c         band ( i in (j-ml,j+mu) ) with the lowest diagonal at
c$$$c         the bottom row (row lowd). See details below for this format.
c$$$c
c$$$c ml	= integer. equal to the bandwidth of the strict lower part of A
c$$$c mu	= integer. equal to the bandwidth of the strict upper part of A
c$$$c         if job=1 on entry then these two values are internally computed.
c$$$c
c$$$c lowd  = integer. row number in abd where the lowest diagonal 
c$$$c         (leftmost) of A is located on return. In case lowd = 0
c$$$c         on return, then it is defined to ml+mu+1 on return and the
c$$$c         lowd will contain this value on return. `
c$$$c
c$$$c ierr  = integer. used for error messages. On return:
c$$$c         ierr .eq. 0  :means normal return
c$$$c         ierr .eq. -1 : means invalid value for lowd. (either .lt. 0
c$$$c         or larger than nabd).
c$$$c         ierr .eq. -2 : means that lowd is not large enough and as 
c$$$c         result the matrix cannot be stored in array abd. 
c$$$c         lowd should be at least ml+mu+1, where ml and mu are as
c$$$c         provided on output.
c$$$c
c$$$c----------------------------------------------------------------------* 
c$$$c Additional details on banded format.  (this closely follows the      *
c$$$c format used in linpack. may be useful for converting a matrix into   *
c$$$c this storage format in order to use the linpack  banded solvers).    * 
c$$$c----------------------------------------------------------------------*
c$$$c             ---  band storage format  for matrix abd ---             * 
c$$$c uses ml+mu+1 rows of abd(nabd,*) to store the diagonals of           *
c$$$c a in rows of abd starting from the lowest (sub)-diagonal  which  is  *
c$$$c stored in row number lowd of abd. the minimum number of rows needed  *
c$$$c in abd is ml+mu+1, i.e., the minimum value for lowd is ml+mu+1. the  *
c$$$c j-th  column  of  abd contains the elements of the j-th column of a, *
c$$$c from bottom to top: the element a(j+ml,j) is stored in  position     *
c$$$c abd(lowd,j), then a(j+ml-1,j) in position abd(lowd-1,j) and so on.   *
c$$$c Generally, the element a(j+k,j) of original matrix a is stored in    *
c$$$c position abd(lowd+k-ml,j), for k=ml,ml-1,..,0,-1, -mu.               *
c$$$c The first dimension nabd of abd must be .ge. lowd                    *
c$$$c                                                                      *
c$$$c     example [from linpack ]:   if the original matrix is             *
c$$$c                                                                      *
c$$$c              11 12 13  0  0  0                                       *
c$$$c              21 22 23 24  0  0                                       *
c$$$c               0 32 33 34 35  0     original banded matrix            *
c$$$c               0  0 43 44 45 46                                       *
c$$$c               0  0  0 54 55 56                                       *
c$$$c               0  0  0  0 65 66                                       *
c$$$c                                                                      *
c$$$c then  n = 6, ml = 1, mu = 2. lowd should be .ge. 4 (=ml+mu+1)  and   *
c$$$c if lowd = 5 for example, abd  should be:                             *
c$$$c                                                                      *
c$$$c untouched --> x  x  x  x  x  x                                       *
c$$$c               *  * 13 24 35 46                                       *
c$$$c               * 12 23 34 45 56    resulting abd matrix in banded     *
c$$$c              11 22 33 44 55 66    format                             *
c$$$c  row lowd--> 21 32 43 54 65  *                                       *
c$$$c                                                                      *
c$$$c * = not used                                                         *
c$$$c                                                                      
c$$$*
c$$$c----------------------------------------------------------------------*
c$$$c first determine ml and mu.
c$$$c----------------------------------------------------------------------- 
c$$$      ierr = 0
c$$$c-----------
c$$$      if (job .eq. 1) call getbwd(n,a,ja,ia,ml,mu)
c$$$      m = ml+mu+1
c$$$      if (lowd .eq. 0) lowd = m
c$$$      if (m .gt. lowd)  ierr = -2
c$$$      if (lowd .gt. nabd .or. lowd .lt. 0) ierr = -1
...
c$$$      mdiag = lowd-ml
c$$$      do 30 i=1,n
c$$$         do 20 k=ia(i),ia(i+1)-1
c$$$            j = ja(k)
c$$$            abd(i-j+mdiag,j) = a(k) 
c$$$ 20      continue
c$$$ 30   continue
c$$$      return
c$$$c------------- end of csrbnd ------------------------------------------- 
...
c----------------------------------------------------------------------- 
c$$$      subroutine csrsss (nrow,a,ja,ia,sorted,diag,al,jal,ial,au)
c$$$      real*8 a(*),al(*),diag(*),au(*) 
c$$$      integer ja(*),ia(nrow+1),jal(*),ial(nrow+1)
c$$$      logical sorted 
c$$$c-----------------------------------------------------------------------
c$$$c Compressed Sparse Row     to     Symmetric Sparse Skyline   format 
c$$$c----------------------------------------------------------------------- 
c$$$c this subroutine converts a matrix stored in csr format into the 
c$$$c Symmetric sparse skyline   format. This latter format assumes that 
c$$$c that the matrix has a symmetric pattern. It consists of the following 
c$$$c * the diagonal of A stored separately in diag(*);
c$$$c * The strict lower part of A is stored  in csr format in al,jal,ial 
c$$$c * The values only of strict upper part as stored in csc format in au. 
c$$$c----------------------------------------------------------------------- 
c$$$c On entry
c$$$c-----------
c$$$c nrow  = dimension of the matrix a.
c$$$c a     = real array containing the nonzero values of the matrix 
c$$$c         stored rowwise.
c$$$c ja    = column indices of the values in array a
c$$$c ia    = integer array of length n+1 containing the pointers to
c$$$c         beginning of each row in arrays a, ja.
c$$$c sorted= a logical indicating whether or not the elements in a,ja,ia
c$$$c         are sorted. 
c$$$c 
c$$$c On return
c$$$c --------- 
c$$$c diag  = array containing the diagonal entries of A
c$$$c al,jal,ial = matrix in csr format storing the strict lower 
c$$$c              trangular part of A.
c$$$c au    = values of the strict upper trangular part of A, column wise.
c$$$c----------------------------------------------------------------------- 
c$$$c 
c$$$c     extract lower part and diagonal.
c$$$c
c$$$      kl = 1
c$$$      ial(1) = kl
c$$$      do  7 i=1, nrow
c$$$c
c$$$c scan all elements in a row
c$$$c 
c$$$         do 71 k = ia(i), ia(i+1)-1
c$$$            jak = ja(k) 
c$$$            if (jak  .eq. i) then
c$$$               diag(i) = a(k) 
c$$$            elseif (jak .lt. i) then
c$$$               al(kl) = a(k)
c$$$               jal(kl) = jak
c$$$               kl = kl+1
c$$$            endif
...
c$$$ 7    continue
c$$$c
c$$$c sort if not sorted
c$$$c 
c$$$      if (.not. sorted) then
c$$$         call csort (nrow, al, jal, ial, au, .true.) 
c$$$      endif
c$$$c
c$$$c copy u
c$$$c 
...
c-----------------------------------------------------------------------
c$$$      subroutine csrvbr(n,ia,ja,a,nr,nc,kvstr,kvstc,ib,jb,kb,
c$$$     &     b, job, iwk, nkmax, nzmax, ierr )
c$$$c-----------------------------------------------------------------------
c$$$      integer n, ia(n+1), ja(*), nr, nc, ib(*), jb(nkmax-1), kb(nkmax)
c$$$      integer kvstr(*), kvstc(*), job, iwk(*), nkmax, nzmax, ierr
c$$$      real*8  a(*), b(nzmax)
c$$$c-----------------------------------------------------------------------
c$$$c     Converts compressed sparse row to variable block row format.
c$$$c-----------------------------------------------------------------------
c$$$c     On entry:
c$$$c--------------
c$$$c     n       = number of matrix rows
c$$$c     ia,ja,a = input matrix in CSR format
c$$$c
c$$$c     job     = job indicator.
c$$$c               If job=0, kvstr and kvstc are used as supplied.
c$$$c               If job=1, kvstr and kvstc are determined by the code.
c$$$c               If job=2, a conformal row/col partitioning is found and
c$$$c               returned in both kvstr and kvstc.  In the latter two cases,
c$$$c               an optimized algorithm can be used to perform the
c$$$c               conversion because all blocks are full.
c$$$c
c$$$c     nkmax   = size of supplied jb and kb arrays
c$$$c     nzmax   = size of supplied b array
c$$$c
c$$$c     If job=0 then the following are input:
c$$$c     nr,nc   = matrix block row and block column dimension
c$$$c     kvstr   = first row number for each block row
c$$$c     kvstc   = first column number for each block column.
c$$$c               (kvstr and kvstc may be the same array)
c$$$c
c$$$c     On return:
c$$$c---------------
c$$$c
c$$$c     ib,jb,kb,b = output matrix in VBR format
c$$$c
c$$$c     ierr    = error message
c$$$c               ierr = 0 means normal return
c$$$c               ierr = 1 out of space in jb and/or kb arrays
c$$$c               ierr = 2 out of space in b array
c$$$c               ierr = 3 nonsquare matrix used with job=2
c$$$c
c$$$c     If job=1,2 then the following are output:
c$$$c     nr,nc   = matrix block row and block column dimension
c$$$c     kvstr   = first row number for each block row
c$$$c     kvstc   = first column number for each block column
c$$$c               If job=2, then kvstr and kvstc contain the same info.
c$$$c
c$$$c     Work space:
c$$$c----------------
c$$$c     iwk(1:ncol) = inverse kvstc array.  If job=1,2 then we also need:
c$$$c     iwk(ncol+1:ncol+nr) = used to help determine sparsity of each block row.
c$$$c     The workspace is not assumed to be initialized to zero, nor is it
c$$$c     left that way.
c$$$c
c$$$c     Algorithms:
c$$$c----------------
c$$$c     There are two conversion codes in this routine.  The first assumes
c$$$c     that all blocks are full (there is a nonzero in the CSR data
c$$$c     structure for each entry in the block), and is used if the routine
c$$$c     determines the block partitioning itself.  The second code makes
c$$$c     no assumptions about the block partitioning, and is used if the
c$$$c     caller provides the partitioning.  The second code is much less
c$$$c     efficient than the first code.
c$$$c
c$$$c     In the first code, the CSR data structure is traversed sequentially
c$$$c     and entries are placed into the VBR data structure with stride
c$$$c     equal to the row dimension of the block row.  The columns of the
c$$$c     CSR data structure are sorted first if necessary.
c$$$c
c$$$c     In the second code, the block sparsity pattern is first determined.
c$$$c     This is done by traversing the CSR data structure and using an
c$$$c     implied linked list to determine which blocks are nonzero.  Then
c$$$c     the VBR data structure is filled by mapping each individual entry
c$$$c     in the CSR data structure into the VBR data structure.  The columns
c$$$c     of the CSR data structure are sorted first if necessary.
c$$$c
c$$$c-----------------------------------------------------------------------
c$$$c     Local variables:
c$$$c---------------------
c$$$      integer ncol, nb, neqr, numc, a0, b0, b1, k0, i, ii, j, jj, jnew
c$$$      logical sorted
c$$$c
c$$$c     ncol = number of scalar columns in matrix
c$$$c     nb = number of blocks in conformal row/col partitioning
c$$$c     neqr = number of rows in block row
c$$$c     numc = number of nonzero columns in row
c$$$c     a0 = index for entries in CSR a array
c$$$c     b0 = index for entries in VBR b array
c$$$c     b1 = temp
c$$$c     k0 = index for entries in VBR kb array
c$$$c     i  = loop index for block rows
c$$$c     ii = loop index for scalar rows in block row
c$$$c     j  = loop index for block columns
c$$$c     jj = loop index for scalar columns in block column
c$$$c     jnew = block column number
c$$$c     sorted = used to indicate if matrix already sorted by columns
c$$$c
c$$$c-----------------------------------------------------------------------
c$$$      ierr = 0
c$$$c-----sort matrix by column indices
c$$$      call csorted(n, ia, ja, sorted)
c$$$      if (.not. sorted) then
c$$$         call csort (n, a, ja, ia, b, .true.)
c$$$      endif
c$$$      if (job .eq. 1 .or. job .eq. 2) then
c$$$c--------need to zero workspace; first find ncol
c$$$         ncol = 0
c$$$         do i = 2, n
c$$$            ncol = max0(ncol, ja(ia(i)-1))
c$$$         enddo
c$$$         do i = 1, ncol
c$$$            iwk(i) = 0
c$$$         enddo
c$$$         call csrkvstr(n, ia, ja, nr, kvstr)
c$$$         call csrkvstc(n, ia, ja, nc, kvstc, iwk)
c$$$      endif
c$$$c-----check if want conformal partitioning
c$$$      if (job .eq. 2) then
c$$$         if (kvstr(nr+1) .ne. kvstc(nc+1)) then
c$$$            ierr = 3
c$$$            return
c$$$         endif
c$$$c        use iwk temporarily
c$$$         call kvstmerge(nr, kvstr, nc, kvstc, nb, iwk)
c$$$         nr = nb
c$$$         nc = nb
c$$$         do i = 1, nb+1
c$$$            kvstr(i) = iwk(i)
...
c$$$      endif
c$$$c-----------------------------------------------------------------------
c$$$c     inverse kvst (scalar col number) = block col number
c$$$c     stored in iwk(1:n)
c$$$c-----------------------------------------------------------------------
c$$$      do i = 1, nc
c$$$         do j = kvstc(i), kvstc(i+1)-1
c$$$            iwk(j) = i
c$$$         enddo
c$$$      enddo
c$$$      ncol = kvstc(nc+1)-1
c$$$c-----jump to conversion routine
c$$$      if (job .eq. 0) goto 400
c$$$c-----------------------------------------------------------------------
c$$$c     Fast conversion for computed block partitioning
c$$$c-----------------------------------------------------------------------
c$$$      a0 = 1
c$$$      b0 = 1
c$$$      k0 = 1
c$$$      kb(1) = 1
c$$$c-----loop on block rows
c$$$      do i = 1, nr
c$$$         neqr = kvstr(i+1) - kvstr(i)
c$$$         numc = ia(kvstr(i)+1) - ia(kvstr(i))
c$$$         ib(i) = k0
c$$$c--------loop on first row in block row to determine block sparsity
c$$$         j = 0
c$$$         do jj = ia(kvstr(i)), ia(kvstr(i)+1)-1
c$$$            jnew = iwk(ja(jj))
c$$$            if (jnew .ne. j) then
c$$$c--------------check there is enough space in kb and jb arrays
c$$$               if (k0+1 .gt. nkmax) then
c$$$                  ierr = 1
c$$$                  write (*,*) 'csrvbr: no space in kb for block row ', i
c$$$                  return
c$$$               endif
c$$$c--------------set entries for this block
c$$$               j = jnew
c$$$               b0 = b0 + neqr * (kvstc(j+1) - kvstc(j))
c$$$               kb(k0+1) = b0
c$$$               jb(k0) = j
c$$$               k0 = k0 + 1
c$$$            endif
c$$$         enddo
c$$$c--------loop on scalar rows in block row
c$$$         do ii = 0, neqr-1
c$$$            b1 = kb(ib(i))+ii
c$$$c-----------loop on elements in a scalar row
c$$$            do jj = 1, numc
c$$$c--------------check there is enough space in b array
c$$$               if (b1 .gt. nzmax) then
c$$$                  ierr = 2
c$$$                  write (*,*) 'csrvbr: no space in b for block row ', i
c$$$                  return
c$$$               endif
c$$$               b(b1) = a(a0)
c$$$               b1 = b1 + neqr
...
c$$$      return
c$$$c-----------------------------------------------------------------------
c$$$c     Conversion for user supplied block partitioning
c$$$c-----------------------------------------------------------------------
c$$$ 400  continue
c$$$c-----initialize workspace for sparsity indicator
c$$$      do i = ncol+1, ncol+nc
c$$$         iwk(i) = 0
c$$$      enddo
c$$$      k0 = 1
c$$$      kb(1) = 1
c$$$c-----find sparsity of block rows
c$$$      do i = 1, nr
c$$$         neqr = kvstr(i+1) - kvstr(i)
c$$$         numc = ia(kvstr(i)+1) - ia(kvstr(i))
c$$$         ib(i) = k0
c$$$c--------loop on all the elements in the block row to determine block sparsity
c$$$         do jj = ia(kvstr(i)), ia(kvstr(i+1))-1
c$$$            iwk(iwk(ja(jj))+ncol) = 1
c$$$         enddo
c$$$c--------use sparsity to set jb and kb arrays
c$$$         do j = 1, nc
c$$$            if (iwk(j+ncol) .ne. 0) then
c$$$c--------------check there is enough space in kb and jb arrays
c$$$               if (k0+1 .gt. nkmax) then
c$$$                  ierr = 1
c$$$                  write (*,*) 'csrvbr: no space in kb for block row ', i
c$$$                  return
c$$$               endif
c$$$               kb(k0+1) = kb(k0) + neqr * (kvstc(j+1) - kvstc(j))
c$$$               jb(k0) = j
c$$$               k0 = k0 + 1
c$$$               iwk(j+ncol) = 0
c$$$            endif
...
c$$$      enddo
c$$$      ib(nr+1) = k0
c$$$c-----Fill b with entries from a by traversing VBR data structure.
c$$$      a0 = 1
c$$$c-----loop on block rows
c$$$      do i = 1, nr
c$$$         neqr = kvstr(i+1) - kvstr(i)
...
c$$$         do ii = 0, neqr-1
c$$$            b0 = kb(ib(i)) + ii
c$$$c-----------loop on block columns
c$$$            do j = ib(i), ib(i+1)-1
c$$$c--------------loop on scalar columns within block column
c$$$               do jj = kvstc(jb(j)), kvstc(jb(j)+1)-1
c$$$c-----------------check there is enough space in b array
c$$$                  if (b0 .gt. nzmax) then
c$$$                     ierr = 2
c$$$                     write (*,*)'csrvbr: no space in b for blk row',i
c$$$                     return
c$$$                  endif
c$$$                  if (a0 .ge. ia(kvstr(i)+ii+1)) then
c$$$                     b(b0) = 0.d0
c$$$                  else
c$$$                     if (jj .eq. ja(a0)) then
c$$$                        b(b0) = a(a0)
c$$$                        a0 = a0 + 1
c$$$                     else
c$$$                        b(b0) = 0.d0
...
c$$$                  endif
c$$$                  b0 = b0 + neqr
c$$$c--------------endloop on scalar columns
c$$$               enddo
c$$$c-----------endloop on block columns
c$$$            enddo
c$$$ 2020       continue
c$$$         enddo
c$$$      enddo
...
c$$$c----------------------------end-of-csrvbr------------------------------
c$$$c----------------------------------------------------------------------c
c$$$      subroutine vbrcsr(ia, ja, a, nr, kvstr, kvstc, ib, jb, kb,
c$$$     &   b, nzmax, ierr)
c$$$c-----------------------------------------------------------------------
c$$$      integer ia(*), ja(*), nr, ib(nr+1), jb(*), kb(*)
c$$$      integer kvstr(nr+1), kvstc(*), nzmax, ierr
c$$$      real*8  a(*), b(nzmax)
c$$$c-----------------------------------------------------------------------
c$$$c     Converts variable block row to compressed sparse row format.
c$$$c-----------------------------------------------------------------------
c$$$c     On entry:
c$$$c--------------
c$$$c     nr      = number of block rows
c$$$c     kvstr   = first row number for each block row
c$$$c     kvstc   = first column number for each block column
c$$$c     ib,jb,kb,b = input matrix in VBR format
c$$$c     nzmax   = size of supplied ja and a arrays
c$$$c
c$$$c     On return:
c$$$c---------------
c$$$c     ia,ja,a = output matrix in CSR format
c$$$c
c$$$c     ierr    = error message
c$$$c               ierr = 0 means normal return
c$$$c               ierr = negative row number when out of space in
c$$$c                      ja and a arrays
c$$$c
c$$$c     Work space:
c$$$c----------------
...
c$$$c     Algorithm:
c$$$c---------------
c$$$c     The VBR data structure is traversed in the order that is required
c$$$c     to fill the CSR data structure.  In a given block row, consecutive
c$$$c     entries in the CSR data structure are entries in the VBR data
c$$$c     structure with stride equal to the row dimension of the block.
c$$$c     The VBR data structure is assumed to be sorted by block columns.
c$$$c
c$$$c-----------------------------------------------------------------------
c$$$c     Local variables:
c$$$c---------------------
c$$$      integer neqr, numc, a0, b0, i, ii, j, jj
c$$$c
c$$$c     neqr = number of rows in block row
c$$$c     numc = number of nonzero columns in row
c$$$c     a0 = index for entries in CSR a array
...
c$$$c-----loop on block rows
c$$$      do i = 1, nr
c$$$c--------set num of rows in block row, and num of nonzero cols in row
c$$$         neqr = kvstr(i+1) - kvstr(i)
c$$$         numc = ( kb(ib(i+1)) - kb(ib(i)) ) / neqr
c$$$c--------construct ja for a scalar row
c$$$         do j = ib(i), ib(i+1)-1
c$$$            do jj = kvstc(jb(j)), kvstc(jb(j)+1)-1
c$$$               ja(a0) = jj
c$$$               a0 = a0 + 1
c$$$            enddo
c$$$         enddo
c$$$c--------construct neqr-1 additional copies of ja for the block row
c$$$         do ii = 1, neqr-1
c$$$            do j = 1, numc
c$$$               ja(a0) = ja(a0-numc)
c$$$               a0 = a0 + 1
c$$$            enddo
c$$$         enddo
c$$$c--------reset a0 back to beginning of block row
c$$$         a0 = kb(ib(i))
c$$$c--------loop on scalar rows in block row
c$$$         do ii = 0, neqr-1
c$$$            ia(kvstr(i)+ii) = a0
...
c$$$c-----------loop on elements in a scalar row
c$$$            do jj = 1, numc
c$$$c--------------check there is enough space in a array
c$$$               if (a0 .gt. nzmax) then
c$$$                  ierr = -(kvstr(i)+ii)
c$$$                  write (*,*) 'vbrcsr: no space for row ', -ierr
c$$$                  return
c$$$               endif
c$$$               a(a0) = b(b0)
c$$$               a0 = a0 + 1
