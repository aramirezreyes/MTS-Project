...
!!   B(k)=Tk
     ...
     REAL(kind=8) :: cl1,cl2,val_cl,g1
     INTEGER :: k,j,pp1,pp2,type,Zone_Phys_Bord1,Zone_Phys_Bord2,lig,NN,iadd
 ...
 allocate(V1(NN),V2(NN))
  ...
  do j=1,mesh%nbLines
     ...
     if (type.eq.Zone_Phys_Bord1) then
       val_CL=mesh%POS(lig,2)
     else if (type.eq.Zone_Phys_Bord2) then
       val_CL=CL2
     end if
     !! - One multiplies the k-th column of the stiffness matrix by the
     !!  value Tk, and one subtracts it to the nodal forces vector.
